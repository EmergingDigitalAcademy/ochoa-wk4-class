...
// for (begin; condition; step) {...code block...}

const positives = [];

// loop supplyChanges to log each value in the array
for (let i = 0; i < supplyChanges.length; i++) {
  // need to index the supplyChanges array to get the value and that index, ie, supplyChanges[0] will be 3
  //console.log(supplyChanges[0]) will be 3
  //console.log(supplyChanges[1]) will be 5
  // and so on...
  //...
  console.log(supplyChanges[i]);
  // collect just the postive numbers
  if (supplyChanges[i] > 0) {
    positives.push(supplyChanges[i]);
  }
}
// console.log the positives array
console.log(positives);

// write a for..of loop using the positives array

const aboveSix = [];
for (const number of positives) {
  // for...of loops have access to the value, not the index
  console.log(number);
  if (number > 6) {
    aboveSix.push(number);
  }
}
console.log('values greater than 6', aboveSix);
...
while (counter > 0) {
  ...
  // need to increment/decrement our counter
  // counter is evaluated for truethy on each iteration
  counter--;
}
